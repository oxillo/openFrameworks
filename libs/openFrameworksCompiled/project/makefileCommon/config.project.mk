...
ifneq ($(strip $(CORE_PKG_CONFIG_LIBRARIES)),)
	ifeq ($(CROSS_COMPILING),1)
		 ADDONS_PKG_CONFIG_LIBS=$(shell export PKG_CONFIG_LIBDIR=$(PKG_CONFIG_LIBDIR);$(PKG_CONFIG) "$(CORE_PKG_CONFIG_LIBRARIES)" --libs)
		 OF_CORE_LIBS += $(patsubst /%,$(SYSROOT)/%,$(ADDONS_PKG_CONFIG_LIBS))
	else
		OF_CORE_LIBS += $(shell $(PKG_CONFIG) "$(CORE_PKG_CONFIG_LIBRARIES)" --libs)
	endif
endif
...
ifneq ($(strip $(PROJECT_ADDONS_PKG_CONFIG_LIBRARIES)),)
	ifeq ($(CROSS_COMPILING),1)
		OF_CORE_INCLUDES_CFLAGS += $(patsubst -I%,-I$(SYSROOT)% ,$(shell export PKG_CONFIG_LIBDIR=$(PKG_CONFIG_LIBDIR);$(PKG_CONFIG) "$(CORE_PKG_CONFIG_LIBRARIES)" --cflags))
	else
