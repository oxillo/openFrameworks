...
define parse_addons_libraries
	...
	$(if $(PARSED_ALL_PLATFORM_LIBS), \
		...
		$(eval PARSED_ADDONS_LIBS_PLATFORM_LIBS_SHARED = $(shell $(FIND) $(PARSED_ALL_PLATFORM_LIBS) -name *.so 2> /dev/null | grep -v "/\.[^\.]" | sed 's/ /\\ /g')) \
		$(eval PARSED_ADDONS_LIBS_PLATFORM_LIBS_SHARED += $(shell $(FIND) $(PARSED_ALL_PLATFORM_LIBS) -name *.dylib 2> /dev/null | grep -v "/\.[^\.]" | sed 's/ /\\ /g')) \
		$(eval PARSED_ADDONS_LIBS_PLATFORM_LIBS_SHARED += $(shell $(FIND) $(PARSED_ALL_PLATFORM_LIBS) -name *.dll 2> /dev/null | grep -v "/\.[^\.]" | sed 's/ /\\ /g')) \
...
ifdef MAKEFILE_DEBUG
    ...
    $(call esp-foreach-info,$(PROJECT_ADDONS_PATHS))
    $(info ---PROJECT_ADDONS_WITH_CONFIG---)
    $(call esp-foreach-info,$(PROJECT_ADDONS_WITH_CONFIG))
    $(info ---PROJECT_ADDONS_INCLUDES---)
    $(call esp-foreach-info,$(PROJECT_ADDONS_INCLUDES))
    $(info ---PROJECT_ADDONS_SOURCE_FILES---)
    $(call esp-foreach-info,$(PROJECT_ADDONS_SOURCE_FILES))
    $(info ---PROJECT_ADDONS_LIBS---)
    $(call esp-foreach-info,$(PROJECT_ADDONS_LIBS))
    $(info ---PROJECT_ADDONS_OBJFILES---)
    $(call esp-foreach-info,$(PROJECT_ADDONS_OBJFILES))
    $(info ---PROJECT_ADDONS_BASE_CFLAGS---)
    $(call esp-foreach-info,$(PROJECT_ADDONS_BASE_CFLAGS))
    $(info ---PROJECT_ADDONS_DEFINES_CFLAGS---)
    $(call esp-foreach-info,$(PROJECT_ADDONS_DEFINES_CFLAGS))
    $(info ---PROJECT_ADDONS_INCLUDES_CFLAGS---)
    $(call esp-foreach-info,$(PROJECT_ADDONS_INCLUDES_CFLAGS))
    $(info ---PROJECT_ADDONS_LDFLAGS---)
    $(call esp-foreach-info,$(PROJECT_ADDONS_LDFLAGS))
    $(info ---PROJECT_ADDONS_DATA---)
    $(call esp-foreach-info,$(PROJECT_ADDONS_DATA))
endif
